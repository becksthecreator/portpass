...
import {
  ...
  createBootstrapWeddingAdmin,
  makeWeddingStaffToken,
...
export async function POST(request: Request) {
  ...
  try {
    await createBootstrapWeddingAdmin({ name, accountKey, pin });
    const token = await makeWeddingStaffToken(accountKey, pin);
    if (!token) throw new Error("Could not sign in the new account.");
    ...
    if (message === "ALREADY_BOOTSTRAPPED") return NextResponse.json({ error: "An account already exists. Sign in instead." }, { status: 409 });
    ...
    console.error("Wedding bootstrap-admin error", error);

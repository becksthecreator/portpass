...
export function AccountsManager({ initialAccounts, currentAccountKey }: { initialAccounts: StaffAccount[]; currentAccountKey: string }) {
  ...
  async function toggleActive(account: StaffAccount) {
    setError("");
    const response = await fetch("/api/weddings/admin/accounts", {
      method: "PATCH",
      ...
      <div className="team-admin-panels">
        <form className="team-admin-form" onSubmit={create}>
          ...
          <div className="team-form-two">
            <label><span>Name</span><input name="name" placeholder="Wedding Desk" required /></label>
          ...
          <div className="team-form-two">
            <label><span>Role</span>
              <select name="role" required defaultValue="wedding_desk">
                <option value="wedding_desk">Wedding Desk — leads, packages, gallery, content</option>
